...
<header @class([
    "relative  pt-7 border-b border-slate-300 dark:border-customBlack-dark bg-blue-400 shadow-md dark:shadow-none bg-center bg-no-repeat-y",
    ...
    <x-container class="flex flex-col lg:flex-row items-center justify-around h-full">
        <div class="flex flex-col gap-2 lg:gap-5">
            ...
            <div class="onlines-count bg-customGreen-dark w-auto px-4 rounded-lg h-10 relative dark:bg-customBlack-dark flex items-center justify-center">
                <div class="absolute h-2 w-2 bg-customGreen-dark dark:bg-customBlack-dark rotate-45 -top-1 left-1/2 -translate-x-1/2"></div>
                <span class="text-sm text-white dark:text-slate-100">{{ __(":c connected habbo's", ['c' => User::whereOnline('1')->count()]) }}</span>
            </div>
        </div>
        @auth
