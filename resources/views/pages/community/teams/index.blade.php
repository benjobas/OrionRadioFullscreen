                    ...
                    >
                        ...
                        <div class="w-full grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 bg-white dark:bg-customBlack h-auto rounded-b-lg p-2">
                                    ...
                                    <div class="w-full overflow-hidden bg-customGreen-dark dark:bg-customBlack-light border-b dark:border-customBlack-light relative flex items-center justify-start p-2 h-10 bg-center bg-no-repeat rounded-t-md">
                                        <div class="w-full flex gap-2 justify-start items-center">
                                            <div @class([
                                                ...
                                                "bg-green-400 animate-pulse" => $user->online,
                                                "bg-red-500" => ! $user->online,
                                            ])></div>
                                            <a href="{{ route('users.profile.show', $user->username) }}" class="text-white font-bold truncate text-sm hover:underline underline-offset-4">{{ $user->username }}</a>
                                        </div>
                                        <div
                                            class="absolute -bottom-12 right-0 w-[64px] h-[110px] bg-center bg-no-repeat"
                                            style="background-image: url('{{ getFigureUrl($user->look, 'direction=3&head_direction=2&size=m&gesture=sml') }}')"
                                        ></div>
                                    </div>
                            ...
                            >
                                ...
                                {{ __('No user found for this team.') }}
                            </span>
                            @endforelse
                        </div>
                    </div>
